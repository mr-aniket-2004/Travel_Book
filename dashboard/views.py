...
from .huggedface_api import generate_story
import markdown
from .helper import parse_ai_blog
...
@login_required
def create_Aiblog(request):
    travellist = TravelDiary.objects.filter(user=request.user)
    travellist_id = request.GET.get('blog_title')
    ...
    if travellist_id:
        selected_travel = get_object_or_404(
            TravelDiary,
            id=travellist_id,
            user=request.user
        )

        # ✅ Use travel relation instead of title
        blog_created = blogs.objects.filter(
            user=request.user,
        ...
        if not blog_created:
            ...
            travellist_dic = {
                "title": selected_travel.title,
                "about": selected_travel.location,
                "start_date": selected_travel.start_date,
                "end_date": selected_travel.end_date,
                "description": selected_travel.description,
                "highlights": selected_travel.highlights,
                "type": selected_travel.categories.name if selected_travel.categories else None,
            }

            raw_blog = generate_story(travellist_dic)
            parsed = parse_ai_blog(raw_blog)

            if parsed:
                blog_created = blogs.objects.create(
                    user=request.user,
                    travel=selected_travel,  # 🔥 Important
                    title=parsed["title"],
    ...
    # UPDATE BLOG
    # -------------------------
    if request.method == "POST" and blog_created:
        blog_created.title = request.POST.get('title')
        blog_created.about = request.POST.get('about')
        blog_created.body = request.POST.get('body')
        blog_created.save()
    ...
    body_html = None
    if blog_created:
        body_html = markdown.markdown(blog_created.body, extensions=["extra"])

    return render(request, "Dashboard/add_content/Aiblog.html", {
        "travellist": travellist,
        "travellist_id": int(travellist_id) if travellist_id else None,
...
def create_blog(request):
    types = TravelDiary.objects.all()
    categories = Categories.objects.all()    

    if request.method == "POST":
        memory_id = request.POST.get('memories')
        ...
        memory = get_object_or_404(TravelDiary,id=memory_id,user=request.user)
